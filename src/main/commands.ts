...
import type { Credential, CredentialInput, CredentialSummary, CopyField } from '../shared/types.js';
import { addCredential, getCredential, getAllCredentials, updateCredential, deleteCredential } from './db.js';
import { encrypt, decrypt } from './crypto.js';

export function cmdList(db: Database.Database): CredentialSummary[] {
  return getAllCredentials(db).map(({ id, name, username }) => ({ id, name, username }));
}

export function cmdAdd(db: Database.Database, key: Buffer, input: CredentialInput): number {
...
export function cmdEdit(db: Database.Database, key: Buffer, id: number, input: CredentialInput): boolean {
...
export function cmdCopy(db: Database.Database, key: Buffer, id: number, field: CopyField): string | null {
  const cred = getCredential(db, id);
  if (!cred) return null;
